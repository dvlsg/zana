...
p=p.length?p.type+"("+p.length+")":p.type;0===g?f.push("    "+m.ColumnName+" "+p+(m.IsNullable?"":" NOT NULL")):f.push(m.ColumnName+" "+p+(m.IsNullable?"":" NOT NULL"))}f=f.join("\n    , ");d.push(f);k.push("    , PRIMARY KEY (");for(g=0;g<c.PrimaryKeys.length;g++)h.push(c.PrimaryKeys[g].ColumnName);k.push(h.join(", "));k.push(")");c=k.join("");d.push(c);d.push(")");d=d.join("\n");a.log.log(d)}};e.load=function(c,d,f){a.check.isString(c);a.check.isString(d);a.check.isNumber(f);b=d.toLowerCase();switch(b){case "sqlite":openDatabase(c,
